...
This package ships a [Laravel Boost](https://github.com/laravel/boost) skill, `sharpapi-ecommerce-review-sentiment`, that teaches AI coding agents how to call `EcommerceReviewSentimentService` correctly: submit, then `fetchResults()` in a queued job, check the job status, and read the result. It loads on demand and adds no always-on guideline. Boost 2 or newer is required.
...
Select `sharpapi/laravel-ecommerce-review-sentiment` when Boost lists the packages it found.
